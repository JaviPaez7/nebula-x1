/**
 * Development-only: render the machine from fixed studio angles.
 * Usage: node tools/beauty.mjs --views 0,1,2 [--progress 0.55]
 */
...
const PORT = Number(getArg('--port', 9337))
const W = Number(getArg('--w', 1600))
const H = Number(getArg('--h', 900))
const P = Number(getArg('--progress', 0.55))
const OUT = getArg('--out', 'shots/beauty')
const VIEWS = getArg('--views', '0,1,2,3,4,5').split(',').map(Number)
const sleep = (ms) => new Promise((r) => setTimeout(r, ms))
mkdirSync(OUT, { recursive: true })

const profile = mkdtempSync(join(tmpdir(), 'nebula-beauty-'))
const chrome = spawn(
  CHROME,
  [
...
try {
  ...
  for (const v of VIEWS) {
    await cdp.send('Runtime.evaluate', { expression: `window.__BEAUTY__ = ${v}` })
    await sleep(1400)
    const shot = await cdp.send('Page.captureScreenshot', { format: 'png' })
    const name = `${OUT}/view${v}.png`
    writeFileSync(name, Buffer.from(shot.data, 'base64'))
    console.log('shot', name)
  }
  const errs = cdp.logs.filter((l) => !/404/.test(l))
  console.log('--- logs ---')
